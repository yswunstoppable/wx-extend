...
class WxValidate {
     ...
     * @param {Boolean} isAsync 是否为异步函数
     */
    addMethod(name, method, message, isAsync = false) {
      // 如果是异步函数则包装成一个object,object.isAsync=true,obejct.method=method
      isAsync
        ? (this.methods[name] = { isAsync: true, method: method })
    ...
    isValidMethod(value) {
      ...
      for (let method in this.methods) {
        // 如果是异步函数那么this.methods[method]对应的是一个object
        if (
          method &&
          ["function", "object"].includes(typeof this.methods[method])
        ) {
          methods.push(method);
        }
      }
    ...
    checkParam(param, rules, data) {
      ...
      for (let method in rules) {
        ...
        if (this.isValidMethod(method)) {
          ...
          // 对异步函数和同步函数分开处理
          // 若为异步函数直接推进Promise数组并continue
          // 若为同步函数则直接根据条件进行判断执行break或continue
  
          if (this.isAsyncMethod(this.methods[method])) {
            this.asyncMethodArray.push(
              new Promise((resolve, reject) => {
                this.methods[method]
                  .method(value, rule.parameters)
                  .then((res) => {
                    resolve();
                  })
                  .catch((err) => {
     ...
     */
    valid() {
      // 这里返回promise对象，代替原来的直接返回Boolean，故在调用checkForm时应采用Promise.then/catch进行判断
      return new Promise((resolve, reject) => {
        Promise.all(this.asyncMethodArray)
          .then((res) => {
            this.size() === 0 ? resolve() : reject();
          })
          .catch((err) => reject());
      });
    }
